...
# Load dataset with ECG signals and labels
...
# Free memory
del x_data, y_data, X_train_reshaped


# Build unimodal model
def build_model():
	...
	y = layers.Flatten()(y)
	y = layers.Dense(512, activation='relu', kernel_initializer='he_uniform')(y)
	y = layers.Dropout(0.5)(y)
	output = layers.Dense(6, activation='softmax', kernel_initializer='glorot_normal')(y)

	model = keras.Model(inputs=input_ecg, outputs=output)
	return model
...
# Results file
results_file = f"/mnt/Home-Group/kruiz_cps/12ECG/unimodal_{selected_leads}/training_results_unimodal_{selected_leads}.txt"
with open(results_file, "w") as f:
	f.write(f"Training results unimodal ({selected_leads})\n\n")

# Cross-validation loop
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train.argmax(axis=1))):
	print(f"\nFold {fold + 1}")

	# Split train/validation for current fold
	...
	checkpoint = callbacks.ModelCheckpoint(
		f'unimodal_{selected_leads}_fold{fold+1}.keras', monitor='val_loss',
	...
	plt.xlabel('Epochs')
	plt.ylabel('Loss')
	plt.title(f'Learning Curves of the Unimodal Model for {num_lead} Leads')
	plt.legend()
	plt.grid()
	plt.savefig(
		f"/mnt/Home-Group/kruiz_cps/12ECG/unimodal_{selected_leads}/loss_curve_unimodal_{selected_leads}_fold{fold+1}.pdf",
		dpi=300
	)
	plt.close()

	# Evaluate best model
	best_model = keras.models.load_model(f'unimodal_{selected_leads}_fold{fold+1}.keras')
	y_pred = best_model.predict(X_val, verbose=0).argmax(axis=1)
